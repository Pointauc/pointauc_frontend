...
): AnalyticsEventMap[typeof analyticsEventNames.wheelSpinResult]['dropout_variant'] => {
...
export const buildWheelSpinResultPayload = ({
  ...
  return {
    ...
    dropout_variant: mapDropoutVariant(settings.format, settings.dropoutVariant),
    randomness_source: settings.randomnessSource,
    configured_spin_time_seconds: settings.spinTime,
    actual_spin_time_seconds: spinDuration,
    is_random_spin_enabled: settings.randomSpinEnabled,
    random_spin_min_seconds: settings.randomSpinEnabled ? settings.randomSpinConfig.min : null,
    random_spin_max_seconds: settings.randomSpinEnabled ? settings.randomSpinConfig.max : null,
    split: settings.split,
    wheel_style: settings.wheelStyles ?? null,
    has_core_image: Boolean(settings.coreImage),
    soundtrack_enabled: Boolean(settings.soundtrack?.enabled),
    soundtrack_source_type: settings.soundtrack?.enabled ? settings.soundtrack?.source?.type ?? 'none' : 'none',
    max_depth: settings.maxDepth,
    depth_restriction: settings.depthRestriction,
    total_participant_count: totalStatistics.participantCount,
    final_spin_participant_count: finalSpinStatistics.participantCount,
    total_weight: totalStatistics.totalWeight,
    final_spin_weight: finalSpinStatistics.totalWeight,
    min_weight: totalStatistics.minWeight,
    max_weight: totalStatistics.maxWeight,
    average_weight: totalStatistics.averageWeight,
    winner_weight: winnerFromAllItems.amount,
    winner_chance_percent_total: calculateWinnerChancePercent(winnerFromAllItems.amount, totalStatistics.totalWeight),
    winner_chance_percent_final_spin: calculateWinnerChancePercent(
